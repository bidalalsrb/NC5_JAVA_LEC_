...
//1. KOR, CHI, JAP, USA 네 개의 상수를 갖는 enum을 생성하고 첫 번째 값은 "나라 한글명"이고
// 두 번째 값은 "대표음식"으로 지정한다.
//   public void notifyFood(String country) 메소드를 만들고
//   country 값과 동일한 한글명인 나라의 음식을
//   다음과 같이 출력하도록 구현하세요.
...
public enum enum1ex {
    ...
    public void notifyFood(String country) {
        if (country.equals(getName())) {
            System.out.println(name + "의" + " 대표음식은 " + food + "입니다");
        }
    }

    enum1ex(String name, String food) {
        this.name = name;
        this.food = food;
    }
